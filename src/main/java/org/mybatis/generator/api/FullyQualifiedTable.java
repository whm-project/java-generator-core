...
public class FullyQualifiedTable {
    ...
    private String queryModelObjectSubPackage;
    ...
    public FullyQualifiedTable(String introspectedCatalog,
        ...
        if (stringHasValue(queryModelObjectName)) {
            int index = queryModelObjectName.lastIndexOf('.');
            if (index == -1) {
                this.queryModelObjectName = queryModelObjectName;
            } else {
                this.queryModelObjectName = queryModelObjectName.substring(index + 1);
                this.queryModelObjectSubPackage = queryModelObjectName.substring(0, index);
            }
        }
    ...
    public String getQueryModelObjectName() {
        if (stringHasValue(queryModelObjectName)) {
            ...
            return getCamelCaseString(runtimeTableName, true)+"QueryModel";
        } else {
            return getCamelCaseString(introspectedTableName, true)+"QueryModel";
        }
    }
    /**
    ...
    public String getSubPackageForQueryModel(boolean isSubPackagesEnabled) {
        StringBuilder sb = new StringBuilder();
        sb.append(getSubPackageForClientOrSqlMap(isSubPackagesEnabled));

        if (stringHasValue(queryModelObjectSubPackage)) {
            sb.append('.');
            sb.append(queryModelObjectSubPackage);
        }
