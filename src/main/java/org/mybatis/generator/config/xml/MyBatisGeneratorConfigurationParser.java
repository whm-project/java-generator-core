...
public class MyBatisGeneratorConfigurationParser {
    ...
    protected void parseTable(Context context, Node node) {
        ...
        String queryModelObjectName = attributes.getProperty("queryModelObjectName"); //$NON-NLS-1$
        ...
        if (stringHasValue(queryModelObjectName)) {
            tc.setQueryModelObjectName(queryModelObjectName+"QueryModel");
        }

        if (stringHasValue(queryModelObjectName)) {
            tc.setServiceObjectName(queryModelObjectName);
        }
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("deleteAffectedTable".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseDeleteAffectedTable(tc, childNode);
            }else if("deletedStateColumn".equals(childNode.getNodeName())){
                parseDeleteStateColumn(tc, childNode);
            }else if("checkUnique".equals(childNode.getNodeName())){
                parseCheckUnique(tc, childNode);
            }else if("treeStructure".equals(childNode.getNodeName())){
                parseTreeStructure(tc, childNode);
            }
        }
    }
    ...
    private void parseTreeStructure(TableConfiguration tc, Node node) {
        ...
        if (stringHasValue(isParent)) {
            treeStructure.setIsParentColumnName(isParent);
        }
        tc.setTreeStructure(treeStructure);
    }
     ...
     * @param node
     */
    private void parseCheckUnique(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        CheckUnique cu = new CheckUnique();
        String[] uniqueColumnAry = attributes.getProperty("uniqueColumnAry").split(" ");//$NON-NLS-1$
        for(int i=0; i<uniqueColumnAry.length; i++){
            cu.addUniqueColumn(uniqueColumnAry[i]);
        }
        tc.addCheckUniqueList(cu);
    }

    /**
     * 设置如果是状态删除，删除状态字段及删除状态值
     * @param tc
     * @param node
     */
    private void parseDeleteStateColumn(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String value = attributes.getProperty("value"); //$NON-NLS-1$
        DeleteStateColumn dsc = new DeleteStateColumn(column, value);
        tc.setDeleteStateColumn(dsc);
    }
     ...
     * @param node
     */
    private void parseDeleteAffectedTable(TableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        DeleteAffectedTable dat = new DeleteAffectedTable();

        String affectedTable = attributes.getProperty("affectedTable"); //$NON-NLS-1$
        dat.setAffectedTableName(affectedTable);

        String[] sourceColumnAry = attributes.getProperty("sourceColumnAry").split(" ");//$NON-NLS-1$
        String[] affectedColumnAry = attributes.getProperty("affectedColumnAry").split(" "); //$NON-NLS-1$
        List sourceColumnList = new ArrayList();
        List affectedColumnList = new ArrayList();
        for(int i=0; i<sourceColumnAry.length; i++){
            if(affectedColumnAry.length >i){
                dat.addColumnMatch(sourceColumnAry[i], affectedColumnAry[i]);
            }
        }

        tc.addDeleteAffectedTableList(dat);
    }
